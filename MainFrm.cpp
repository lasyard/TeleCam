...
void CMainFrame::selectCamViewItemByCamId(int camId)
{
	...
	if (hItem != NULL) m_wndCamView.selectItem(hItem);
}
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_wndCamView.Create(strTemp, this, CRect(0, 0, 200, 200), TRUE, ID_VIEW_CAM_VIEW,
		WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI))
	{
		ATLTRACE2("Failed to create Cam View window.\n");
		return -1;
	}
	HICON hCamViewIcon = (HICON)::LoadImage(::AfxGetResourceHandle(),
		MAKEINTRESOURCE(IDI_CAM_VIEW), IMAGE_ICON,
	...
	if (!m_wndOutput.Create(strTemp, this, CRect(0, 0, 100, 100), TRUE, ID_VIEW_OUTPUTWND,
	...
	HICON hOutputBarIcon = (HICON)::LoadImage(::AfxGetResourceHandle(),
		MAKEINTRESOURCE(IDI_OUTPUT_WND), IMAGE_ICON,
		::GetSystemMetrics(SM_CXSMICON), ::GetSystemMetrics(SM_CYSMICON), 0);
	...
	if ((m_hDevNotify = RegisterDeviceNotification(
		m_hWnd,
		&dev,
...
{
	if (m_hDevNotify != NULL) UnregisterDeviceNotification(m_hDevNotify);
}
...
{
	if (nEventType != DBT_DEVICEARRIVAL && nEventType != DBT_DEVICEREMOVECOMPLETE) return TRUE;
	PDEV_BROADCAST_HDR pHdr = (PDEV_BROADCAST_HDR)dwData;
	if (pHdr->dbch_devicetype != DBT_DEVTYP_DEVICEINTERFACE) return TRUE;
	PDEV_BROADCAST_DEVICEINTERFACE pDev = (PDEV_BROADCAST_DEVICEINTERFACE)dwData;
	if (pDev->dbcc_classguid != AM_KSCATEGORY_CAPTURE) return TRUE;
	CString temp;
...
{
	CCam *pCam = getSelectedCam();
	if (pCam == NULL) return;
	if (!pCam->hasWindow()) {
		CChildFrame *pChild = new CChildFrame;
		if (!pChild->Create(NULL, pCam->name, WS_CHILD | WS_VISIBLE | WS_OVERLAPPEDWINDOW,
			rectDefault, this, NULL))
		{
...
void CMainFrame::OnCloseCam()
{
	CCam *pCam = getSelectedCam();
	if (pCam == NULL) return;
	if (pCam->hasWindow()) pCam->closeWindow();
}
...
CCam *CMainFrame::getSelectedCam()
{
