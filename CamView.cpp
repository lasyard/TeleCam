...
BEGIN_MESSAGE_MAP(CCamView, CDockablePane)
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_WM_CONTEXTMENU()
	ON_WM_PAINT()
	ON_WM_SETFOCUS()
	ON_NOTIFY(TVN_SELCHANGED, IDC_CAM_TREE, &CCamView::OnNotifySelChanged)
END_MESSAGE_MAP()
...
void CCamView::showCamList()
{
	...
	const list<CCam *> &camList = theApp.m_pCamManager->getCamList();
	list<CCam *>::const_iterator i;
	int count = 0;
	...
	if (count > 0) m_wndCamView.Expand(hRoot, TVE_EXPAND);
}

CCam *CCamView::getCamFromItem(HTREEITEM h)
{
...
CCam *CCamView::getCamFromSelected()
{
...
HTREEITEM CCamView::getItemByCamId(int camId)
{
	HTREEITEM hItem, h;
	hItem = m_wndCamView.GetRootItem();
	if (hItem == NULL) return NULL;
	for (; ;) {
		if ((int)m_wndCamView.GetItemData(hItem) == camId) return hItem;
...
void CCamView::selectItem(HTREEITEM hItem)
{
	m_wndCamView.SelectItem(hItem);
}

void CCamView::setSelectedItemImage(int index)
{
	HTREEITEM hItem = m_wndCamView.GetSelectedItem();
	if (hItem != NULL) m_wndCamView.SetItemImage(hItem, index, index);
}

BOOL CCamView::OnCmdMsg(UINT nID, int nCode, void *pExtra, AFX_CMDHANDLERINFO *pHandlerInfo)
{
	if (GetParent()->OnCmdMsg(nID, nCode, pExtra, pHandlerInfo)) return TRUE;
...
void CCamView::OnContextMenu(CWnd *pWnd, CPoint point)
{
	...
	if (point != CPoint(-1, -1)) {
		...
		if (hTreeItem != NULL && (TVHT_ONITEM & flags)) pWndTree->SelectItem(hTreeItem);
	}
...
void CCamView::OnSetFocus(CWnd *pOldWnd)
{
	CDockablePane::OnSetFocus(pOldWnd);
	m_wndCamView.SetFocus();
}

void CCamView::OnNotifySelChanged(NMHDR *pNotifyStruct, LRESULT *result)
{
	LPNMTREEVIEW pNm = (LPNMTREEVIEW)pNotifyStruct;
	if (pNm == NULL) return;
	TVITEM tvNew = pNm->itemNew;
	TVITEM tvOld = pNm->itemOld;
	if (tvOld.hItem != NULL) m_wndCamView.SetItemState(tvOld.hItem, 0, TVIS_BOLD);
	if (tvNew.hItem != NULL) {
		m_wndCamView.SetItemState(tvNew.hItem, TVIS_BOLD, TVIS_BOLD);
		CCam *pCam = getCamFromItem(tvNew.hItem);
		if (pCam != NULL) pCam->setWindowFocus();
	}
	*result = S_OK;
}
